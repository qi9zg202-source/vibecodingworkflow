...
### Step 1b：自动搜索客户与对标目标背景信息

**触发条件：** 用户填写了「项目名称」和「对标目标」后自动执行，无需用户额外指令。

使用 web search 工具搜索以下内容，搜索完成后将结果整理为摘要展示给用户确认：

**搜索一：客户基本信息**

搜索关键词：`[项目名称/客户名称] 公司介绍 主要产品 规模`
...
**搜索二：对标目标的行业 leader 地位依据**

搜索关键词：`[对标目标] 行业地位 核心竞争力 [所属行业]`
...
搜索关键词：`[对标目标] [功能领域，如"能效管理" / "制造执行系统"] 最佳实践 行业标准`
...
如有不准确或需要补充，请直接告知。确认后我将继续收集功能需求。
```

等待用户确认或修正后，继续 Step 1c。

### Step 1c：读取客户资料文件

**触发条件：** 用户在问卷「客户资料」字段填写了至少一个文件路径。若留空则跳过此步骤，直接进入 Step 2。

> 提示：客户资料文件建议统一放在项目根目录的 `customer_context/` 文件夹下，便于管理和引用。

#### 读取策略

文件可能较大，必须按以下策略分段读取，不得跳过或假设内容：

**结构化文件（Excel / CSV 点位表、电表清单等）：**
1. 先读文件头部（前 50–100 行），提取字段名、列定义、单位
2. 按系统或子系统分批读取数据行（每批不超过 200 行）
3. 每批读完立即提取关键信息，累积汇总
4. 全部读完后合并汇总结果

**非结构化文件（PDF / Word 说明文档）：**
1. 按页码范围分批读取（每批不超过 10 页）
2. 每批提取与本次功能相关的内容
3. 全部读完后合并汇总结果

**读取过程中如遇到无法解析的格式：** 告知用户具体文件名和问题，询问是否提供可读版本（如导出为 CSV），不自行跳过。

#### 提取目标

从文件中提取以下类型的信息（按相关性筛选，不相关的不提取）：
- 系统/子系统名称与边界
- 点位名称、测量对象、单位、采集频率
- 关键指标定义（如 EER、COP、负荷率的计算方式）
- 数据范围或典型值（如正常运行区间）
...
## Project Background
- [基于用户描述 + 搜索结果填写：项目是什么、服务对象、核心业务场景、地点与规模]
...
## Domain Data
> 来源：客户提供的背景资料文件（如点位表、系统说明、电表清单等）
> 若无客户资料则留空或删除此 section
...
[用户] 查看结果，做出决定：
  ...
  IF 用户要求修改（局部调整，如"再改一下 X"/"不对，应该是 Y"）：
      # 判断标准：用户指出具体问题，修改范围在本 Session 子任务之内
      → 在同一窗口继续修改（不需要开新窗口）
      → 修改完成后重新执行测试 Gate
      → 再次等待用户验收

  IF 用户拒绝（整体方向不对，如"重来"/"这个交互逻辑完全不是我想要的"）：
      # 判断标准：用户否定整体方向，或修改范围超出本 Session 子任务定义
      → 不写 summary，不更新 memory.md（Session 未完成，测试通过不等于验收通过）
      → 与用户确认：是修改 tasksubsessionN.md 重新规划，还是调整 PRD.md/task.md 后重新规划
      → 按用户决定修改对应文件后，开新窗口重新执行
```
