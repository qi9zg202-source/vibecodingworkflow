...
| 文件 | 用途 | 客户操作 |
|------|------|----------|
| **1paperprdasprompt.md** | 工作流完整规范，大模型的"操作手册" | 放入项目目录，让 Codex/Claude 读取 |
| **docs/user-guide.md** | 用户手册，含完整示例和常见问题 | 阅读后开始使用 |
...
| R2 | Session 0a 需求问卷与文档产出 | 10 | ✅ passed |
...
**测试覆盖：** 1paperprdasprompt.md line 671–675
...
| 多 Session 禁止行为 | 补了拒绝时的标准回复格式 | SECTION 4 禁止行为 line 741 |
| 需求大改旁路 | 新增三级变更分级 + 大改处理流程 + 复用评估表 | SECTION 3 修订规则 line 663–710 |
...
基于真实业务场景（TSMC 台积电 Fab 厂务平台 — 制冷机房优化策略管理）：
