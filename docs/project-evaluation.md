...
| 评审交付一致性 | 最终原型文件名与封装形式未完全统一，旧示例仍存在 `index.html + styles.css + app.js` 分离模式 | 产品经理分发给开发评审时成本高，交付口径不一致 |
...
#### P0-4：最终评审产物统一为 `task.html` 单文件

**问题：**
当前规范虽已强调 HTML 交付，但仓库内仍混有 `[功能名].html`、`index.html`、`styles.css`、`app.js` 等旧表述，导致最终评审产物的命名和封装形式不统一。产品经理将原型分发给开发评审时，容易出现漏文件、资源失效、版本不一致的问题。

**优化方案：**
- 最终交付文件名固定为 `task.html`
- CSS、JavaScript、模拟数据及小型静态资源全部内联到 `task.html`
- 禁止依赖外部 CDN、禁止拆分 `styles.css` / `app.js` / `core-models.js` 等运行时文件
- 将该约束同步写入 `1paperprdasprompt.md`、用户手册、测试用例和交付清单，形成单一口径
...
- 断网环境下双击 `task.html` 可正常打开
- 产品经理只需分发 `task.html` 即可完成开发评审
